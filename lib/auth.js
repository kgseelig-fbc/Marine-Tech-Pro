...
// live for up to 8 hours of inactivity (the cookie is rolling — see
...
if (googleConfigured) {
    ...
    passport.use(new GoogleStrategy({
        ...
        try {
            ...
            // profile.photos is deliberately not passed through: nothing
            // renders it and the privacy policy does not list it.
            ...
            // null: the email already belongs to another account (typically a
            // local signup). Refuse rather than link or duplicate — see
            // upsertOauthUser. The callback turns this into /login?error=email_exists.
            if (!user) return done(null, false, { message: 'email_exists' });
...
function loadUser(req, res, next) {
    ...
    if (req.session && req.session.userId) {
        ...
        if (u) {
            ...
            // Session points at a deleted user. Strip the id in place — same
            // reasoning as the break-glass branch above: session.destroy()
            // deletes req.session synchronously, and the login/signup/logout
            // handlers that run next all dereference it. With this cookie the
            // first request after an admin deletion used to crash the whole
            // process via an unhandled rejection in the login handler.
...
function requireAdmin(req, res, next) {
    if (!req.user || req.user.role !== 'admin') {
        ...
        // A signed-in tech who lands on an admin page goes back to the app,
        // not to /login — bouncing an authenticated user to the login page
        // reads as "you were signed out". Anonymous callers never get here
        // (requireAuth runs first), but keep the fallback correct anyway.
