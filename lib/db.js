...
// Token spend and its dollar cost since `sinceTs`, split by model.
//
// Grouped by model because the rates differ per model and `ai_messages` spans
// however many models the app has used: one SUM over everything would price a
// mixed window at a single model's rate. Rows written before the `model`
// column are attributed to pricing.LEGACY_MODEL.
//
// tokens_in already contains the cache reads and writes, so fresh input is
// the remainder — priced at the full input rate, while cached tokens are a
// tenth of it and cache writes double it. Billing them all as fresh input
// (the obvious SUM(tokens_in)) overstates a cache-heavy workload like this
// one several-fold.
function getAiUsage(sinceTs = 0) {
    const rows = db.prepare(`
        SELECT COALESCE(model, @legacy) AS model,
               COUNT(*) AS asks,
               COALESCE(SUM(MAX(COALESCE(tokens_in, 0) - COALESCE(cache_read, 0) - COALESCE(cache_write, 0), 0)), 0) AS inputTokens,
               COALESCE(SUM(COALESCE(cache_read, 0)), 0)  AS cacheReadTokens,
               COALESCE(SUM(COALESCE(cache_write, 0)), 0) AS cacheWriteTokens,
               COALESCE(SUM(COALESCE(tokens_out, 0)), 0)  AS outputTokens
        FROM ai_messages
        WHERE ts >= @since
        GROUP BY 1
        ORDER BY 1
    `).all({ legacy: pricing.LEGACY_MODEL, since: sinceTs });
    ...
    const out = {
        ...
        totalTokens: 0,
        costUsd: 0,
        // Asks on a model with no rate in lib/pricing.js. Their tokens are in
        // the totals above but their dollars are in nobody's — surfaced so the
        // dashboard can say the cost is short, instead of quietly under-reporting.
        unpricedAsks: 0,
        byModel: []
    };

    for (const r of rows) {
        const costUsd = pricing.roundUsd(pricing.costOf({
            model: r.model,
        ...
        const totalTokens = r.inputTokens + r.cacheReadTokens + r.cacheWriteTokens + r.outputTokens;
        ...
        if (costUsd === null) out.unpricedAsks += r.asks;
        else out.costUsd += costUsd;

        out.byModel.push({ ...r, totalTokens, costUsd });
    }
    out.costUsd = pricing.roundUsd(out.costUsd);
    return out;
}
...
function getSummary() {
    ...
    return {
        ...
        aiUsage: {
            last24h: getAiUsage(now - day),
            last7d:  getAiUsage(now - 7 * day),
            last30d: getAiUsage(now - 30 * day),
            total:   getAiUsage(0)
        }
    };
}
