...
const stmtInsertAi = db.prepare(`
    INSERT INTO ai_messages(ts, session_id, ip_hash, user_id, question, answer, ctx_tree, ctx_node, tokens_in, tokens_out, cache_read, cache_write, duration_ms, ok, error)
    VALUES (@ts, @session_id, @ip_hash, @user_id, @question, @answer, @ctx_tree, @ctx_node, @tokens_in, @tokens_out, @cache_read, @cache_write, @duration_ms, @ok, @error)
`);
...
function getRecentAi(limit = 25) {
    return db.prepare(`
        SELECT ts, question, answer, ctx_tree, ctx_node, ok, duration_ms, tokens_in, tokens_out, cache_read, cache_write
        FROM ai_messages
        ORDER BY id DESC
        LIMIT ?
    `).all(limit);
