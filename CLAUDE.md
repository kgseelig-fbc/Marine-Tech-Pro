...
- **Cost is derived, never stored.** `lib/pricing.js` holds the list-price rate table and all the money arithmetic; `datastore.getAiUsage(since)` aggregates a window per model and `getRecentAi` prices each row. Two traps it exists to avoid: `tokens_in` **already contains** the cache reads and writes (billing that column whole prices a cache hit as fresh input, ~10× over), and a cache write costs *more* than fresh input (2× at the 1h TTL), so the cheap and expensive paths differ by ~25× per question. A model with no rate is reported as `unpricedAsks`, never as $0. Rows predating the `model` column are priced as `pricing.LEGACY_MODEL`, a frozen constant, so a model switch cannot reprice history.
