...
Also honoured: `RETENTION_DAYS` (telemetry pruning window, default 90), `ASK_RATE_PER_MIN` (Ask-a-Tech questions per minute per user, default 15), `SIGNUP_RATE_PER_HOUR` (successful local sign-ups per hour per IP, default 10), `DRAIN_TIMEOUT_MS` (default 25000), `GIT_SHA`. `README.md` and `.env.example` are the operator-facing reference for all of these.
