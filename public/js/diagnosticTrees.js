...
window.defined_trees = {
    ...
    engine_runs_rough: {
        ...
        nodes: {
            ...
            fix_plugs: { id: "fix_plugs", type: "resolution", severity: "LOW", title: "Replace Spark Plugs", text: "Spark plugs are fouled, worn, or wrong heat range.", action: "Replace ALL spark plugs with the correct part number and heat range for the engine (see Engine Specs). Gap to spec. Apply anti-seize sparingly to threads. Torque to spec. Inspect old plugs for clues — oil fouling, fuel fouling, overheat, or detonation marks.", partsNeeded: ["Spark plugs (full set, correct part number)"], estimatedTime: "30-60 minutes" },

            fix_coils: { id: "fix_coils", type: "resolution", severity: "MEDIUM", title: "Replace Ignition Coil(s)", text: "Ignition coil is breaking down under load or heat.", action: "Replace the failed coil(s). Inspect the coil boots and plug wires for cracks or carbon tracking — replace if suspect. Verify primary and secondary resistance on remaining coils against service-manual spec. Clear codes and test under load.", partsNeeded: ["Ignition coil(s)", "Coil boots if cracked"], estimatedTime: "1-2 hours" },

            low_compression: { id: "low_compression", type: "resolution", severity: "HIGH", title: "Low Compression", text: "One or more cylinders have low compression.", action: "Run a wet vs dry compression test to localize: rings vs valves. If the low cylinder rises with oil, suspect rings. If unchanged, suspect valves or head gasket. Notify customer — this is a major repair (top-end teardown). Do NOT return to service.", warning: "Continued running with low compression can mask other failures and damage the engine further.", partsNeeded: ["Top-end gasket set or rebuild parts as identified"], estimatedTime: "Diagnostic 1-2 hours; repair varies (8+ hours)" },

            low_fuel_pressure: { id: "low_fuel_pressure", type: "resolution", severity: "MEDIUM", title: "Low Fuel Pressure", text: "Fuel pressure is below spec.", action: "Verify pressure with a gauge at the rail. Compare to spec for this engine. If low: replace the primary and high-pressure filters, inspect the lift pump and high-pressure pump, check for restrictions in the supply line, and verify the tank vent is open. Replace the failed pump or regulator as identified.", partsNeeded: ["Fuel filters", "Fuel pump (if failed)", "Pressure regulator (if failed)"], estimatedTime: "1-3 hours" },
            ...
            fix_vent: { id: "fix_vent", type: "resolution", severity: "LOW", title: "Tank Vent Repair", text: "Fuel tank vent is restricted.", action: "Locate the vent fitting (usually on the hull side). Inspect for mud-dauber wasps, debris, or kinked hose. Blow out gently with low-pressure air. Confirm the tank can breathe by loosening the deck fill cap — if that restores running, the vent is the cause.", partsNeeded: ["Replacement vent fitting if damaged"], estimatedTime: "30-60 minutes" },

            fix_connector: { id: "fix_connector", type: "resolution", severity: "MEDIUM", title: "Connector Repair", text: "Connector pins are corroded, spread, or pushed back.", action: "Disconnect the connector. Inspect each pin for corrosion or damage. Clean with contact cleaner and a small brush. Tighten spread female pins with a pin pick. Replace the connector if pins are damaged or housing is cracked. Apply dielectric grease before reassembly.", partsNeeded: ["Replacement connector kit (if damaged)", "Contact cleaner", "Dielectric grease"], estimatedTime: "30-90 minutes" },

            oh_fix_sensor: { id: "oh_fix_sensor", type: "resolution", severity: "MEDIUM", title: "Temperature Sensor Repair", text: "Engine temperature sensor is reading incorrectly.", action: "Test the sensor at known temperatures and compare to spec. Replace if out of range. Inspect the connector for corrosion. Apply dielectric grease. Clear codes and verify normal warm-up readings.", partsNeeded: ["Temperature sensor", "Dielectric grease"], estimatedTime: "30-60 minutes" }
        }
    },
    ...
    charging_electrical: {
        ...
        nodes: {
            ...
            fix_simple: { id: "fix_simple", type: "resolution", severity: "LOW", title: "Simple Fix — Clean and Reseat", text: "Connection or part needed cleaning, reseating, or a basic adjustment.", action: "Disconnect the suspect connector or component. Clean contacts with electrical contact cleaner. Apply dielectric grease. Reseat firmly until it clicks. Verify operation. If the issue returns, escalate to a wiring or component-level diagnosis.", partsNeeded: ["Electrical contact cleaner", "Dielectric grease"], estimatedTime: "15-30 minutes" },

            fix_connector: { id: "fix_connector", type: "resolution", severity: "MEDIUM", title: "Connector Repair", text: "Connector pins are corroded, spread, or pushed back.", action: "Disconnect the connector. Inspect each pin for corrosion or damage. Clean with contact cleaner and a small brush. Tighten spread female pins with a pin pick. Replace the connector if pins are damaged or housing is cracked. Apply dielectric grease before reassembly.", partsNeeded: ["Replacement connector kit (if damaged)", "Contact cleaner", "Dielectric grease"], estimatedTime: "30-90 minutes" },

            fix_cables: { id: "fix_cables", type: "resolution", severity: "MEDIUM", title: "Battery / Power Cable Repair", text: "Battery or power cable is corroded or undersized.", action: "Inspect cables end-to-end. Replace any cable with green corrosion under the insulation. Use marine-grade tinned cable sized for the load. Replace lugs with marine-grade tinned ring terminals. Apply dielectric grease at terminals. Torque battery posts to spec.", partsNeeded: ["Marine battery cable (correct gauge)", "Tinned ring terminals", "Dielectric grease"], estimatedTime: "1-2 hours" },

            fix_fuse: { id: "fix_fuse", type: "resolution", severity: "LOW", title: "Replace Blown Fuse", text: "Fuse is blown or fuse holder is corroded.", action: "Replace fuse with the SAME amperage rating. Inspect the fuse holder for corrosion or melting — replace if damaged. If the new fuse blows immediately, there is a short downstream — do NOT keep replacing fuses. Diagnose the short.", partsNeeded: ["Replacement fuse (same rating)", "Replacement fuse holder if corroded"], estimatedTime: "10-20 minutes" },

            fix_ground: { id: "fix_ground", type: "resolution", severity: "MEDIUM", title: "Ground Connection Repair", text: "Ground point is corroded or loose.", action: "Locate the ground stud. Remove the ground lug, sand the contact surface bright. Replace any green or corroded eyelets with new tinned ring terminals. Torque to spec. Apply dielectric grease over the connection. Verify with a voltage drop test (less than 0.2V under load).", partsNeeded: ["Tinned ring terminals", "Dielectric grease", "Sandpaper"], estimatedTime: "30-60 minutes" }
        }
    },
    ...
    stereo_audio: {
        ...
        nodes: {
            ...
            fix_simple: { id: "fix_simple", type: "resolution", severity: "LOW", title: "Simple Fix — Clean and Reseat", text: "Connection, plug, or setting needed cleaning, reseating, or a basic adjustment.", action: "Disconnect the suspect connector or component. Clean contacts with electrical contact cleaner. Apply dielectric grease. Reseat firmly. Verify operation through all sources/zones. If the issue returns, escalate to a wiring or component-level diagnosis.", partsNeeded: ["Electrical contact cleaner", "Dielectric grease"], estimatedTime: "15-30 minutes" },

            fix_ground: { id: "fix_ground", type: "resolution", severity: "MEDIUM", title: "Ground Connection Repair", text: "Stereo or amp ground point is corroded, loose, or sharing a noisy ground.", action: "Run a dedicated ground wire (same gauge as power) from the head unit and amp to a clean, sanded ground point near the battery negative. Avoid grounding to engine block or noisy chassis points. Apply dielectric grease at the ground stud.", partsNeeded: ["Marine-grade ground wire", "Tinned ring terminals", "Dielectric grease"], estimatedTime: "30-90 minutes" },

            fix_connector: { id: "fix_connector", type: "resolution", severity: "MEDIUM", title: "Connector Repair", text: "Audio or power connector is corroded or intermittent.", action: "Disconnect the connector. Inspect each pin for corrosion or damage. Clean with contact cleaner. Tighten spread pins with a pin pick. Replace the connector or pigtail if pins are damaged. Apply dielectric grease before reassembly.", partsNeeded: ["Replacement connector or pigtail", "Contact cleaner", "Dielectric grease"], estimatedTime: "30-90 minutes" },

            fix_maintainer: { id: "fix_maintainer", type: "resolution", severity: "LOW", title: "Install Battery Maintainer", text: "Stereo memory plus dock time is killing the battery.", action: "Install a marine-rated dock-side battery maintainer (smart charger). Educate customer to plug in when boat is not in use. Consider adding a battery cutoff switch as a backup.", partsNeeded: ["Marine smart battery maintainer", "Battery cutoff switch (optional)"], estimatedTime: "1-2 hours" }
        }
    },
    ...
    nav_lights: {
        ...
        nodes: {
            ...
            fix_wiring: { id: "fix_wiring", type: "resolution", severity: "MEDIUM", title: "Wiring Repair", text: "Nav-light wiring is damaged, chafed, corroded, or shorted.", action: "Trace the affected circuit end-to-end. Repair or replace damaged sections with marine-grade tinned wire. Use heat-shrink butt connectors. Apply dielectric grease at any reopened connections. Re-secure runs with cushioned clamps away from heat and moving parts.", partsNeeded: ["Marine tinned wire (matching gauge)", "Heat-shrink butt connectors", "Dielectric grease"], estimatedTime: "1-3 hours" },

            fix_ground: { id: "fix_ground", type: "resolution", severity: "MEDIUM", title: "Ground Connection Repair", text: "Light fixture ground is corroded or open.", action: "At the fixture, find the ground (black/yellow) wire. Cut back to clean copper, install a new tinned ring terminal, ground to a clean, sanded point. Apply dielectric grease. Verify with a voltage drop test (less than 0.2V under load).", partsNeeded: ["Tinned ring terminals", "Dielectric grease", "Sandpaper"], estimatedTime: "30-60 minutes" },

            fix_simple: { id: "fix_simple", type: "resolution", severity: "LOW", title: "Simple Fix — Clean and Reseat", text: "Bulb, socket, or plug needed cleaning or reseating.", action: "Pull the bulb (or fixture plug). Clean contacts with electrical contact cleaner and a brush. Apply a small amount of dielectric grease. Reseat firmly. Verify operation.", partsNeeded: ["Electrical contact cleaner", "Dielectric grease"], estimatedTime: "10-20 minutes" },

            fix_connector: { id: "fix_connector", type: "resolution", severity: "MEDIUM", title: "Connector Repair", text: "Connector pins are corroded, spread, or pushed back.", action: "Disconnect the connector. Inspect each pin for corrosion or damage. Clean with contact cleaner. Tighten spread female pins with a pin pick. Replace the connector if pins are damaged or housing is cracked. Apply dielectric grease before reassembly.", partsNeeded: ["Replacement connector kit", "Contact cleaner", "Dielectric grease"], estimatedTime: "30-90 minutes" }
        }
    },
    ...
    horn_system: {
        ...
        nodes: {
            ...
            fix_ground: { id: "fix_ground", type: "resolution", severity: "MEDIUM", title: "Ground Connection Repair", text: "Horn ground is corroded or loose.", action: "Locate the horn ground lug. Remove, sand the contact surface bright. Replace any green or corroded eyelet with a new tinned ring terminal. Torque. Apply dielectric grease. Verify with a voltage drop test (less than 0.2V under load).", partsNeeded: ["Tinned ring terminals", "Dielectric grease", "Sandpaper"], estimatedTime: "30-60 minutes" },

            fix_wiring: { id: "fix_wiring", type: "resolution", severity: "MEDIUM", title: "Wiring Repair", text: "Horn circuit wiring is damaged, chafed, corroded, or shorted.", action: "Trace the circuit from button to relay to horn. Repair or replace damaged sections with marine-grade tinned wire (16 AWG is typical for horns). Use heat-shrink butt connectors. Apply dielectric grease at reopened connections.", partsNeeded: ["Marine tinned wire (16 AWG)", "Heat-shrink butt connectors", "Dielectric grease"], estimatedTime: "1-2 hours" },

            fix_simple: { id: "fix_simple", type: "resolution", severity: "LOW", title: "Simple Fix — Clean and Reseat", text: "Connection or button needed cleaning, reseating, or a basic adjustment.", action: "Disconnect the suspect connector. Clean contacts with electrical contact cleaner. Apply dielectric grease. Reseat firmly. Verify operation. If the issue returns, escalate to a wiring or component-level diagnosis.", partsNeeded: ["Electrical contact cleaner", "Dielectric grease"], estimatedTime: "15-30 minutes" },

            fix_connector: { id: "fix_connector", type: "resolution", severity: "MEDIUM", title: "Connector Repair", text: "Connector pins are corroded, spread, or pushed back.", action: "Disconnect the connector. Inspect each pin. Clean with contact cleaner. Tighten spread female pins with a pin pick. Replace the connector if pins are damaged or housing is cracked. Apply dielectric grease before reassembly.", partsNeeded: ["Replacement connector kit", "Contact cleaner", "Dielectric grease"], estimatedTime: "30-90 minutes" }
        }
    },
    ...
    yamaha_flash_codes: {
        ...
        nodes: {
            ...
            fix_lanyard: { id: "fix_lanyard", type: "resolution", severity: "LOW", title: "Lanyard Switch Fix", text: "Lanyard kill switch is open or stuck.", action: "Verify the lanyard clip is fully seated. Inspect the switch for corrosion or sticky operation. Replace the switch if it does not return to the closed position with the clip installed. Apply dielectric grease at the connector. Clear codes and retest.", partsNeeded: ["Lanyard kill switch", "Dielectric grease"], estimatedTime: "30-60 minutes" },

            fix_fuse: { id: "fix_fuse", type: "resolution", severity: "LOW", title: "Replace Blown Fuse", text: "Main or sub fuse is blown.", action: "Replace fuse with the SAME amperage rating (F115: 20A & 30A typical). Inspect the fuse holder for corrosion or melting — replace if damaged. If the new fuse blows immediately, there is a short downstream — diagnose before replacing again.", partsNeeded: ["Replacement fuse (same rating)", "Replacement fuse holder if corroded"], estimatedTime: "10-20 minutes" },

            fix_wiring: { id: "fix_wiring", type: "resolution", severity: "MEDIUM", title: "Wiring Repair", text: "Wiring is damaged, chafed, corroded, or shorted.", action: "Trace the affected circuit end-to-end. Repair or replace damaged sections with marine-grade tinned wire. Use heat-shrink butt connectors. Apply dielectric grease at any reopened connections. Re-secure runs with cushioned clamps. Clear codes and retest.", partsNeeded: ["Marine tinned wire (matching gauge)", "Heat-shrink butt connectors", "Dielectric grease"], estimatedTime: "1-3 hours" },

            fix_cables: { id: "fix_cables", type: "resolution", severity: "MEDIUM", title: "Battery / Power Cable Repair", text: "Battery or power cable is corroded or undersized.", action: "Inspect cables end-to-end. Replace any cable with green corrosion under the insulation. Use marine-grade tinned cable sized for the load. Replace lugs with marine-grade tinned ring terminals. Apply dielectric grease at terminals. Torque battery posts to spec.", partsNeeded: ["Marine battery cable (correct gauge)", "Tinned ring terminals", "Dielectric grease"], estimatedTime: "1-2 hours" },

            fix_neutral_sw: { id: "fix_neutral_sw", type: "resolution", severity: "MEDIUM", title: "Neutral Safety Switch Fix", text: "Neutral safety switch is mis-adjusted or failed.", action: "Verify the shifter is fully in neutral. Adjust the switch actuator per service manual. Test continuity through the switch in neutral and out of neutral. Replace the switch if it does not change state cleanly. Clear codes.", partsNeeded: ["Neutral safety switch (if failed)"], estimatedTime: "1-2 hours" }
        }
    }
